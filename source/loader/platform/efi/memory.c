...
static bool is_suitable_range(efi_memory_descriptor_t *range, phys_size_t size,
	phys_size_t align, phys_ptr_t min_addr, phys_ptr_t max_addr,
	unsigned flags, efi_physical_address_t *_phys)
{
	...
	phys_ptr_t *_phys)
{
	...
	for(i = 0; i < num_entries; i++) {
		if(!is_suitable_range(&memory_map[i], size, align, min_addr,
...
void efi_memory_init(void) {
	...
	for(i = 0; i < num_entries; i++) {
		...
		dprintf(" 0x%016" PRIxPHYS "-0x%016" PRIxPHYS "\n",
			...
			memory_map[i].physical_start
				+ (memory_map[i].num_pages * EFI_PAGE_SIZE));
	}
